...
def _location(text):
    ...
    # longitude and latitude to address
        ...
        if len(loc) == 0:
            ...
            location = _similarity(text, loc).lower()
...
def text_processing(input_df):
    ...
    for i in range(len(input_df["keyword"])):
        try:
            if "%20" in input_df['keyword'].iloc[i]:
                input_df['keyword'].iloc[i].replace("%20", " ")
        except:
            pass
    ...
    df = pd.DataFrame(tweets, columns =['keyword', 'location', 'text', 'target'])
